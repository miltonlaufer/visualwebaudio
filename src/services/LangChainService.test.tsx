...
describe('LangChainService', () => {
  ...
  describe('system prompt', () => {
    it('should include detailed utility node information', () => {
      ...
      // Check for essential information in the simplified prompt
      expect(systemPrompt).toContain('CRITICAL RULES')
      expect(systemPrompt).toContain('AudioDestinationNode')
      expect(systemPrompt).toContain('SliderNode')
      expect(systemPrompt).toContain('MidiToFreqNode')
      expect(systemPrompt).toContain(
        'SliderNode(value=69) → MidiToFreqNode.midiNote → OscillatorNode.frequency'
      )
      expect(systemPrompt).toContain('NO UNCONNECTED NODES ALLOWED')
      expect(systemPrompt).toContain('NODE ID HANDLING')
      expect(systemPrompt).toContain('AUTO-GENERATED by the system')
      expect(systemPrompt).toContain(
        'Use generic identifiers like "SliderNode1", "OscillatorNode1"'
      )
      expect(systemPrompt).toContain('PRO AUDIO ENGINEERING')
      expect(systemPrompt).toContain('DelayNode: ALWAYS add delayTime control')
      expect(systemPrompt).toContain('BiquadFilterNode: Add frequency control')
      expect(systemPrompt).toContain('LABELING: Set "label" property for ALL utility nodes')
      expect(systemPrompt).toContain('updateProperty')
